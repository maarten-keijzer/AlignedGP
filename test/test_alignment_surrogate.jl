...
#@testset "compute_added_value" begin
    ...
    @testset "constant shift needed, all hits" begin
        evals   = [1.0, 2.0, 3.0]
        targets = IntervalVector([intervaltype(5.0, 7.0), intervaltype(6.0, 8.0), intervaltype(7.0, 9.0)])
        ...
        @test hitvector(updated, targets) == [true, true, true]
    end

    @testset "zero is the optimal constant" begin
        evals   = [3.0, 4.0, 5.0]
        targets = IntervalVector([intervaltype(2.0, 5.0), intervaltype(3.0, 6.0), intervaltype(4.0, 7.0)])
        av, updated = compute_added_value(evals, targets)
    ...
    @testset "Inf in evals → graceful miss for that point" begin
        ...
        @test hitvector(updated, targets) == [true, false, true]
    end
...
@testset "align_node" begin
    ...
    inputs  = [[1.0, 2.0, 3.0]]
    targets = IntervalVector((5.0, 7.0), (6.0, 8.0), (7.0, 9.0))
    ...
    @testset "fresh Var node" begin
        ...
        @test hitvector(new_outputs, targets) == [true, true, true]
    end
